...
def requette_3(df):
    st.subheader("Requette 3 : Nombre d'élèves")
...
def requette_4(df):
    st.subheader("Requette 4 : Notes Controle 1")
...
def requette_5(df):
    st.subheader("Requette 5 : Notes Moyenne")
...
def requette_6(df):
    st.subheader("Requette 6 : Mentions")
...
def top_students_by_subclass(df, subclass):
    ...
    top_students_df = sorted_df.head(3)[['Code Massar','Nom et prénom', 'Moyenne']]

    return top_students_df

def requette_7(df):
    st.subheader("Requette 7 : Trois premiers éleves")
